...
def mainGame():
    ...
    global x, y, time, shot, msgTime, msgText, rotationAngle
    ...
    if shot:
        glPushMatrix()
        glTranslate(ball.X, ball.Y, 0)  # Shift by (ball.X, ball.Y)
        glRotate(rotationAngle, 0, 0, 1)
        rotationAngle += 10
        glTranslate(-ball.X, -ball.Y, 0)  # Shift back by (-ball.X, -ball.X)
        ball.draw_rect_tex(getBallTex())  # Redraw
        glPopMatrix()
    ...
    if shot:
        ...
        if ball.bottom > 0 and not CollidedWithMiddleWall() and ball.left > 0 and ball.right < WINDOW_WIDTH:  # Keep moving
            ...
            po = objPath(x, y, force, radians(theta),
                         time)  ## gets its current position after the passed time using Newton's Eqns of motion
            for i in np.arange(theta, theta + 360, 1):
                glPushMatrix()
                glTranslate(0, 0, 0)
                glRotate(i, 1, 0, 0)
                # glTranslatef(po[0], po[1], 0)
